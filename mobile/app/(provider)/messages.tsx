...
export default function MessagesScreen() {
  ...
  const fetchRooms = async () => {
    try {
      console.log('[Provider] Fetching chat rooms...');
      const rooms = await chatService.getRooms();
      console.log('[Provider] Rooms fetched:', JSON.stringify(rooms));
      setChats(rooms);
    } catch (error) {
      console.error('[Provider] Error fetching rooms:', error);
    }
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      <View style={{ height: 110, backgroundColor: BLUE, paddingTop: 50, paddingHorizontal: 20, borderBottomRightRadius: 20, borderBottomLeftRadius: 20 }}>
        <Text style={{ color: '#FFFFFF', fontSize: 24, fontWeight: '800' }}>Pesan Masuk</Text>
      </View>
      <ScrollView 
        style={{ flex: 1 }} 
      ...
      >
        {chats?.map((room) => {
          ...
          const name = user ? `${user.first_name} ${user.last_name}`.trim() || 'Klien' : 'Klien';
